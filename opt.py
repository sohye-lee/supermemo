...
def get_portvals(allocs, prices):
 ...
 port_val = pos_vals.sum(axis=1) # add code here to compute daily portfolio
values
 return port_val
...
def optimize_portfolio(sd=dt.datetime(2008,1,1), ed=dt.datetime(2009,1,1), \
 ...
 allocs = np.ones(len(syms)) / len(syms) # add code here to find the
allocations

 port_val = get_portvals(allocs, prices)
 # Get daily portfolio value
 daily_return = daily_returns(port_val) # add code here to compute daily
portfolio values
 conds = ({'type': 'eq', 'fun': lambda x : 1 - np.sum(x)},
 {'type': 'eq', 'fun': lambda x : 0.9999999 + np.sum(x)})
This study source was downloaded by 100000806582403 from CourseHero.com on 01-27-2024 10:56:46 GMT -06:00
https://www.coursehero.com/file/69775548/optimizationpypy/
 ...
 allocs = minnie_the_mooch.x
 port_val = get_portvals(allocs, prices)
 # Get daily portfolio value
 daily_return = daily_returns(port_val) # add code here to compute daily
portfolio values

 sr = shiggity_sharpe(allocs, prices)
...
def test_code():
 ...
 # Do not assume that any variables defined here are available to your
function/code
 ...
 allocations, cr, adr, sddr, sr = optimize_portfolio(sd = start_date, ed =
end_date,\
 syms = symbols, \
 gen_plot = False)
 # Print statistics
